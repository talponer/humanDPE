...
name="hg19_K6_s1" # Run-name for output filenames
...
# K=6; s=1.4 # define again if starting from here
...
polyY <- c(-0.05,-0.05,-0.25,-0.25)
pdf('classesCluster.pdf', width=10)
plot(hcd, leaflab='none', yaxt='n')
abline(h=h, col='red', lwd=3)
##abline(h=2.5, lty=2)
par(xpd=T)
polygon(x=c(17,65,65,17), y=polyY, col='skyblue', border='black')
polygon(x=c(67,108,108,67), y=polyY, col='skyblue', border='black')
polygon(x=c(109,138,138,109), y=polyY, col='skyblue', border='black')
polygon(x=c(150,197,197,150), y=polyY, col='skyblue', border='black')
polygon(x=c(206,255,255,206), y=polyY, col='skyblue', border='black')
polygon(x=c(283,300,300,283), y=polyY, col='skyblue', border='black')
dev.off()
...
for(i in 1:K) {
    M=matrix(sprintf("%6.4f", t(Motifs[[i]])), ncol=4)
